...
import copy
from models.KDE.KDEstimator import KDEstimator
...
class BayesClassifier():
    ...
    def fit(self, x_train: np.ndarray, y_train: np.ndarray):
        # common among all fit methods
        self.labels = np.unique(y_train)
        # will be set within the fit methods
        self.priors = np.empty((len(self.labels),))
        match self.mode:
            case 'QDA' | 'LDA' | 'GNB':
    ...
    def fit_kde(self, x_train: np.ndarray, y_train: np.ndarray):
        ...
        self.kde_list = [copy.deepcopy(self.kde_config)
    ...
    def fit_gmm(self, x_train: np.ndarray, y_train: np.ndarray):
        ...
        raise NotImplementedError(
            "Gaussian Mixture Models will still be covered in the future")
    ...
    def fit_qda_and_friends(self, x_train: np.ndarray, y_train: np.ndarray):
        ...
        for k, label in enumerate(self.labels):
            ...
            self.covs[k] = np.cov(x_train_given_class.T,
                                  bias=(self.mode != 'QDA'))
            self.priors[k] = x_train_given_class.shape[0] / x_train.shape[0]

        # handle special cases
        if self.mode == 'LDA':
            self.weighted_cov = np.sum(
                self.covs * self.priors.reshape(-1, 1, 1), axis=0)
            del self.covs
        elif self.mode == 'GNB':
            self.covs = np.array([cov * np.eye(cov.shape[0])
    ...
    def predict_proba_x(self, x: np.ndarray) -> np.ndarray:
        prob_product = np.array(
            [self.P(x, k) * prior for k, prior in enumerate(self.priors)])
