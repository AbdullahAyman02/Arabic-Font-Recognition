...
# Function to plot original and modified images
...
def hough_transforms(image: np.ndarray) -> int:
    """
    Detects the orientation of the text in an image.
...
def hough_transforms_with_plotting_stages(image: np.ndarray) -> int:
    """
    Detects the orientation of the text in an image and plots the stages of the process.
...
def rotate_image(rotation_angle: int, image: np.ndarray) -> np.ndarray:
    """
    Rotates an image by a given angle.

    Parameters:
    - rotation_angle (int): The rotation angle.
    - image (numpy.ndarray): The input image.
...
def crop_image_on_boundary_box(binaryimage: np.ndarray) -> np.ndarray:
    """
    Crops an image based on boundary box containing the text.

    Parameters:
    - binaryimage (numpy.ndarray): The binary image.
